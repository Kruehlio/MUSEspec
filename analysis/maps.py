...
def getRGB(planes, minval=None, maxval=None, scale='lin', ddyn=1):
    ...
    if len(planes) != 3:
        ...
        raise SystemExit
    if (planes[0].shape != planes[1].shape) or \
       (planes[1].shape != planes[2].shape):
...
def getSFR(s3d, sC=1, EBV=1):
    ...
    if s3d.ebvmap is not None and EBV == 1:
...
def getOH(s3d, meth='o3n2', sC=1, ra=None, dec=None, rad=None):
    ...
    if ra is not None and dec is not None and rad is not None:
        cenprop, avgprop, medprop, stdprop =\
...
def getBPT(s3d, snf=5, snb=5, sC=0, xlim1=-1.65, xlim2=0.3, ylim1=-1, ylim2=1,
           ra=None, dec=None, racen=None, deccen=None, pHII=True, pGal=False):
    ...
    if pHII:
        ax1.plot(np.log10(np.nansum(nii[sel3])/np.nansum(ha[sel3])),
                 np.log10(np.nansum(oiii[sel3])/np.nansum(hb[sel3])),
                 'o', ms=10,
    ...
    if pGal:
        ax1.plot(np.log10(np.nansum(nii[sel2])/np.nansum(ha[sel2])),
                 np.log10(np.nansum(oiii[sel2])/np.nansum(hb[sel2])),
                 'o', ms=10,
    ...
    if ra is not None and dec is not None:
        ...
        ax1.plot(np.log10(niigrb/hagrb),
                 ...
                 color='black', mec='white', mew=2,
                 label=r'$\mathrm{GRB\,site}$')

    if racen is not None and deccen is not None:
        try:
            posx, posy = s3d.skytopix(racen, deccen)
        except TypeError:
            posx, posy = s3d.sexatopix(racen, deccen)
        ...
        ax1.plot(np.log10(niigrb/hagrb),
                 np.log10(oiiigrb/hbgrb), '*', ms=18,
                 color='red', mec='grey', mew=2,
                 label=r'$\mathrm{Galaxy\,center}$')
    ...
    ax1.plot(x, kf3,  '-', lw=1.5, color='0.0', zorder=-1)

    # AGN/SF discrimination at z = 0 Kewley et al., 2013
    ax1.plot(kf0, kfz0, '--', lw=2.5, color='0.2', zorder=-1)
...
def _gaussfit(x, y):
    gaussparams = onedgaussfit(x, y, params=[np.median(y[0:5]),
                                             np.nanmax(y), np.median(x), 2])
...
def getVel(s3d, line='ha', dv=250, R=2500):
    ...
    for y in range(s3d.data.shape[1]):   # np.arange(100, 200, 1)
        result = Parallel(n_jobs=1, max_nbytes='1G',)\
                         (delayed(_gaussfit)(subwl, fitcube[:, y, i])
                             for i in range(s3d.data.shape[2]))
                ...
                for i in np.arange(-r, r+1, 1):
                    if r-1 < (j**2 + i**2)**0.5 <= r:
                        ...
                        if posy < maxy and posx < maxx:
                            ...
                            # Below threshold, should stop growing the region
