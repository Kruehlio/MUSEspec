...
def _weighted_centroid(x, y, density):
    ...
    """
    mass = np.nansum(density)
    xBar = np.nansum(x*density)/mass
    yBar = np.nansum(y*density)/mass
...
def _roundness(x, y, pixelSize):
    ...
    xBar, yBar = np.nanmean(x), np.nanmean(y)  # Geometric centroid here!
    maxDistance = np.sqrt(np.nanmax((x - xBar)**2 + (y - yBar)**2))
...
def _accretion(x, y, signal, noise, targetSN, pixelSize, quiet, sn_func):
    ...
    if pixelSize is None:
        pixelSize = np.nanmin(distance.pdist(np.column_stack([x, y])))

    currentBin = np.nanargmax(signal/noise)  # Start from the pixel with highest S/N
    ...
    maxnum = int(np.nansum((signal[w]/noise[w])**2)/targetSN**2 + np.nansum(~w))
    ...
    for ind in range(1, n+1):
        ...
        while True:
            ...
            k = np.nanargmin((x[unBinned] - xBar)**2 + (y[unBinned] - yBar)**2)

            # (1) Find the distance from the closest pixel to the current bin
            #
            minDist = np.nanmin((x[currentBin] - x[unBinned[k]])**2 \
            ...
            #
            xBar, yBar = np.nanmean(x[currentBin]), np.nanmean(y[currentBin])
        ...
        xBar, yBar = np.nanmean(x[binned]), np.nanmean(y[binned])
...
def _reassign_bad_bins(classe, x, y):
    ...
    #
    bad = classe == 0
    index = np.nanargmin((x[bad, None] - xnode)**2 + (y[bad, None] - ynode)**2, axis=1)
...
def _cvt_equal_mass(x, y, signal, noise, xnode, ynode, quiet, wvt, sn_func):
    ...
    for it in range(1, xnode.size):  # Do at most xnode.size iterations
        ...
        #
        if x.size < 10000:
            classe = np.nanargmin(((x[:, None] - xnode)**2 + (y[:, None] - ynode)**2)/scale**2, axis=1)
        else:  # use for loop to reduce memory usage
            classe = np.zeros(x.size, dtype=int)
            for j, (xj, yj) in enumerate(zip(x, y)):
                classe[j] = np.nanargmin(((xj - xnode)**2 + (yj - ynode)**2)/scale**2)
        ...
        diff = np.nansum((xnode - xnodeOld)**2 + (ynode - ynodeOld)**2)
...
def voronoi(x, y, signal, noise, targetSN, logger,
            cvt=False, pixelsize=None, plot=True, quiet=True,
...
#    if not np.all((noise[noise != np.nan] > 0)):# & np.isfinite(noise)):
#        raise ValueError('NOISE must be a positive vector')
    ...
    if np.nanmedian(signal)/np.nanmedian(noise) < targetSN/5:
#        print(np.median(signal)/np.median(noise))
        logger.warning('S/N in the pixels seem low, be careful with output')
    if np.nanmin(signal/noise) > targetSN:
    ...
    #
    noise = noise.clip(np.nanmin(noise[noise > 0])*1e-9)

    logger.info('Bin-accretion...')
    classe, pixelsize = _accretion(x, y, signal, noise, targetSN, pixelsize, quiet, sn_func)
    logger.info('%i initial bins.' %np.nanmax(classe))
    logger.info('Reassign bad bins...')
    xNode, yNode = _reassign_bad_bins(classe, x, y)
    logger.info('%i good bins.', xNode.size)
    if cvt:
        logger.info('Modified Lloyd algorithm...')
        xNode, yNode, scale, it = _cvt_equal_mass(x, y, signal, noise, xNode, yNode, quiet, wvt, sn_func)
        logger.info('%i iterations.' %(xNode.size))
    else:
        scale = 1.
    classe, xBar, yBar, sn, area = _compute_useful_bin_quantities(x, y, signal, noise, xNode, yNode, scale, sn_func)
    w = area == 1
    logger.info('Unbinned pixels: %i / %i' %(np.nansum(w), x.size))
    logger.info('Fractional S/N scatter: %.3f ' %(np.nanstd(sn[~w] - targetSN, ddof=1)/targetSN))
