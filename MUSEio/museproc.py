...
from MUSEspec.utils.astro import airtovac
from MUSEspec.utils.fitter import onedgaussfit
from MUSEspec.analysis.functions import smooth, checkExec
...
class MuseSpec(object):
   ...
   Arguments:
    ...
    def setFiles(self, filen, dAxis=3):
        # Get primary header
    ...
    def makemask(self, sky=None, continuum=True,
        ...
        pixels with Objects are denoted with 1, sky pixels with 0. If wl1 or
        wl2 are not given, uses the full cube
        Optional inputs:
        -sky Skycube
        ...
        if not sky:
            logger.info(
                'Creating the object mask with sigma %s in %i iterations',
                sigma, iterate)
        ...
        if os.path.isfile(mask) and clobber is not False:
            os.remove(mask)
        ...
        if not os.path.isfile(mask):
            ...
            if continuum is True and img2 is not None:
                sel = linesel * contsel
            elif continuum is True and img2 is not None:
                sel = contsel
            elif continuum is False and img2 is not None:
                sel = linesel
            else:
                sel = contsel
            ...
            logger.info('Masking %.2f of all spaxels', self.maskpix)

            if writefits is True:
                hdu = pyfits.HDUList()
                head['NAXIS'] = 2
                for delhead in ['NAXIS3', 'CD3_3', 'CD1_3', 'CD2_3', 'CD3_1',
                                'CD3_2', 'CRPIX3', 'CRVAL3', 'CTYPE3',
                                'CUNIT3']:
                    del head[delhead],
    ...
    def subtractSky(self, skypix=None, order=3, ff=0.03):
        ...
        for image in self.data:
            ...
            for j in np.arange(15):
                try:
                    skyindex = np.random.randint(
                        low=0, high=max(3000, len(workimg)/10))
            ...
            params = onedgaussfit(xfit[:-1], yfit,
                                  params=[0, 1, -1, 10],
                                  fixed=[1, 0, 0, 0],
                                  minpars=[0, 0, -20, 0],
                                  limitedmin=[1, 1, 0, 1])
        ...
        backsmooth, backrms = smooth(back[~np.isnan(back)],
                                     20, window='median', rms=1)
        a = sp.polyfit(self.pix[~np.isnan(back)],
                       backsmooth, order, w=1/backrms)
        ...
        ax.plot(self.wave[~np.isnan(back)], back[~np.isnan(back)],
                'o', mec='grey', ms=2)
        ax.plot(self.wave, backfit, lw=2, color='firebrick')
        ax.plot(self.wave[~np.isnan(back)], backsmooth,
                'o', color='olive', ms=1)

        ax.set_ylim(min(backfit)-abs(max(backfit)),
                    max(backfit)+abs(max(backfit)))
